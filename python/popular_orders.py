...
def best_grammar_most_matched(high, low, name):
    print("\n==================================")
    ...
    countsLow = moves_with_order(low, orders_correctform)
    countsHigh = moves_with_order(high, orders_correctform)

    order = np.argsort(countsLow)
    orderedBins = [bins_correctform[i] for (i) in order[::-1]]
    orderedCountsLow = [countsLow[i] for (i) in order[::-1]]
    orderedCountsHigh = [countsHigh[i] for (i) in order[::-1]]

    bin = pd.Series(orderedBins + orderedBins, name="Word Order")
    cnt = pd.Series(orderedCountsHigh + orderedCountsLow, name="Count")
    cnd = pd.Series(["Low Frmaing" for (i) in range(len(orderedCountsLow))] + ["High Framing" for (i) in range(len(orderedCountsLow))], name="condition")
    ...
    print("Number of inputs in game used = ", sum(orderedCountsHigh))
...
moves_exp1_high_user = moves_by_user(dir+"data/data.json", "HighFraming", correct_form=True, coded=True)
moves_exp1_low_user = moves_by_user(dir+"data/data.json", "LowFraming", correct_form=True, coded=True)
best_grammar_most_matched(moves_exp1_high_user, moves_exp1_low_user,  "explicit-framing")
